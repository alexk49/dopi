...
class TestProcessCSVFile(unittest.TestCase):
    ...
    def test_process_csv_file_success(self, mock_pp, mock_email, mock_write_summary, 
                                     mock_write_metadata, mock_fetch, mock_read_csv):
        test_file = MagicMock(spec=Path)
        test_file.name = "testfile.csv" 
        
        mock_directories = {"QUEUE_DIR": Path("/test/queue"), "FAILURES_DIR": Path("/test/failures"), 
                           "COMPLETE_DIR": Path("/test/complete")}
        
        mock_read_csv.return_value = ("test@example.com", "test.resolver.org", ["10.1234/test1", "10.1234/test2"])
        mock_fetch.return_value = [{"doi": "10.1234/test1", "status": "success"}, {"doi": "10.1234/test2", "status": "error"}]
        ...
        mock_fetch.assert_called_once_with(
            ...
            resolving_host="test.resolver.org"
        )
        ...
        mock_email.assert_called_once_with(
            ...
            filepath=Path("/test/complete/results_summary.csv")
        )
